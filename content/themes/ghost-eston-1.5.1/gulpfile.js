...
var glob = {
  ...
  src : {
    amp     : path.src.less + '/*.amp.less',
    less    : path.src.less + '/!(*.amp)*.less',
...
gulp.task( 'minify-amp', function () {
  ...
  return gulp.src( glob.src.amp )
    .pipe( compile )
    .on( 'error', logError )
    .pipe( rename( function (path) {
      path.basename = distro.name + '-' + path.basename.replace('.amp', '-amp');
      path.extname = '.hbs';
    }))
    .pipe( gulp.dest( path.build.hbs + '/partials/css/' ) );
...
gulp.task( 'watch-src' , function () {
  ...
  gulp.watch( glob.src.lessAll, [ 'compile-less', 'minify-amp' ] );
  gulp.watch( glob.src.jsAll,   [ 'compile-js' ] );

});

// Grouped tasks.
gulp.task( 'compile', [ 'compile-less', 'compile-js', 'minify-amp' ] );
gulp.task( 'minify',  [ 'minify-css', 'minify-amp', 'minify-js' ] );
