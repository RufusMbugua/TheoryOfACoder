...
var Feed = {
  ...
  start : function () {
    ...
    this.fetch().done( function ( response ) {
      ...
      // Check response contains data key and key value is an array
      if ( ( response.hasOwnProperty( this.api.data.response ) && Array.isArray( response[ this.api.data.response ] ) ) || this.api.data.response === undefined ) {
        this.set( this.trim( response ) );
        this.render();
      // If not replicate.fail()
      } else {
        this.$el.remove();
      }
